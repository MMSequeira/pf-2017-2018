...
class Ball {
  ...
  // The force exerted by drag or air resistance is proportional to a drag
  // coefficient having to do with the actual shape of the object, the nature
  // of the fluid (air, for instance), etc:
  final float airDragCoefficientForBalls = 0.01;

  // "The coefficient of restitution (COR) is the ratio of the final to initial
  // relative velocity between two objects after they collide."
  // https://en.wikipedia.org/wiki/Coefficient_of_restitution
  final float wallCoefficientOfRestitutionForBalls = 0.9;
  ...
  void collideWithWall() {
    v.mult(wallCoefficientOfRestitutionForBalls);
  ...
  void applyDrag() {
    // Drag in this case means air resistance.

    // Velocity of the ball relative to velocity of the air (the wind vector):
    PVector rv = PVector.sub(v, wind);

    // Unit vector with the direction of the velocity of the ball relative to
    // the velocity of the air (the wind vector):
    PVector n = rv.normalize();

    // Speed of the ball relative to the air:
    float speed = rv.mag();

    // The force is exerted by the air in the opposite direction of the
    // velocity of the ball relative to the air (hence the - operator). It is
    // proportional to the drag coefficient, to the cross sectional area of the
    // ball, that is, to the square of its radius, and to the speed (actually, if the
    // speed is high, the drag should be proportional to the square of the 
    // speed, but we're ignoring this):
    PVector force = PVector.mult(n, -airDragCoefficientForBalls * sqr(radius) * speed);

    // Since F = ma, a = F/m:
    PVector acceleration = PVector.div(force, m);

    // Make the acceleration due to drag act on the velocity:
    v.add(acceleration);
  }
  ...
  void display() {
    ...
    stroke(#FF0000);
    line(0, 0, v.mag(), 0);
    line(v.mag(), 0, (1 - arrowSize) * v.mag(), arrowSize * v.mag()); 
    line(v.mag(), 0, (1 - arrowSize) * v.mag(), -arrowSize * v.mag()); 

    PVector a = gravity.copy().rotate(-v.heading());
    ...
    stroke(#0000FF);
    line(0, 0, a.mag(), 0);
    line(a.mag(), 0, (1 - arrowSize) * a.mag(), arrowSize * a.mag()); 
    line(a.mag(), 0, (1 - arrowSize) * a.mag(), -arrowSize * a.mag()); 
  ...
  String toString() {
    return "{Ball radius: " + radius + ", m: " + m + ", r: " + r + ", v: " + v + "}";
  }
}